...
class Afinn(object):
    ...
    def __init__(self, language="en", emoticons=False, word_boundary=True, negation_handling=False):
        ...
        negation_handling : bool, optional
            Use negation handling if you want revert the polarity of the word when the sentence is negative
            WARNING: it only works with nltk.sentiment.util.mark_negation
            
            EXAMPLE:
            text_neg=nltk.sentiment.util.mark_negation(" I am not bad")
            print(text_neg)
            "I am not bad_NEG"
            afinn= Afinn(negation_handling=True)
            print(afinn.score(text_neg))
            3.0
            

        """
        self._negation_handling= negation_handling# modified by Alessio Sciacchitano
    ...
    def regex_from_tokens(tokens, word_boundary=True, capture=True):
        ...
        #regex = '(?:' + "|".join(tokens_) + ')'
        regex = '(?:' + "|".join(tokens_) + '|\w+_neg)' #modified by Alessio Sciacchitano
    ...
    def scores_with_pattern(self, text):
        ...
        '''Modifica by Alessio Sciacchitano---Negation Handling'''
        if self._negation_handling:
            scores=[]
            for word in words:
                if word[-4:]=='_neg':
                    word= word[:-4]
                    score_=-self._dict[word]
                else:
                    score_=self._dict[word]
