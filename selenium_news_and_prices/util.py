...
# MODIFICA DEL PACCHETTO AGGIUNGENDO but nor although though unless except even
# But Clause
BUT_CLAUSE = r'(?:^(?:but|nor|although|though|unless|except|even|however|)$)'
BUT_CLAUSE_RE = re.compile(BUT_CLAUSE)
...
def mark_negation(document, double_neg_flip=False, shallow=False, but_clause=False):
    """
    ...
    AGGIUNTO but-clause , cioè se questa incontra but, though, however etc ri-inverte la polarita
    ritorna anche il numero di negazioni all'interno della frase
    """
    if not shallow:
    ...
    for i, word in enumerate(doc):
        ...
        # but-clause implementation by Alessio Sciacchitano
        elif but_clause:
            if neg_scope and BUT_CLAUSE_RE.search(word):
                neg_scope = not neg_scope
            elif neg_scope and not BUT_CLAUSE_RE.search(word):
                doc[i] += '_NEG'
                count_neg +=1
